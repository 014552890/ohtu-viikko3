...
description 'User cannot log in with invalid username/password-combination'

scenario "user can not login with incorrect password", {
    given 'command login selected', {
       userDao = new InMemoryUserDao()
       auth = new AuthenticationService(userDao)
       io = new StubIO("login", "pekka", "vaarasalasana") 
       app = new App(io, auth)
    } 
...
description 'Nonexistent user cannot log in'

scenario "nonexistent user can not login to ", {
    given 'command login selected', {
       userDao = new InMemoryUserDao()
       auth = new AuthenticationService(userDao)
       io = new StubIO("login", "petteri", "petteri") 
       app = new App(io, auth)
    }
